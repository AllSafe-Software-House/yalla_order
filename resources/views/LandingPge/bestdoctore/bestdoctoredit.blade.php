...
<div class="card">
    ...
    @endif
    <div class="card-body">
        <form action="{{ route('bestdoctorupdate',$doctor->id) }}" class="d-grid" enctype="multipart/form-data" method="POST">
            @csrf
            <div class="mb-3">
                <label for="text" class="py-2">Doctor</label>
                <select class="form-control" name="doctore_id" id="">
                    <option value="{{ $doctor->doctore_id }}">name : {{ $doctor->doctore->name }} / Department :  {{ $doctor->doctore->department }} (current doctor)</option>
                    @foreach ($doctoresitem as $data)
                        <option value="{{ $data->id }}">name : {{ $data->name }} / Department :  {{ $data->department }}</option>
                    @endforeach
                </select>
            </div>
            <button type="submit"  class="btn btn-primary btn-sm col-12">Sve change</button>
        </form>
    </div>
</div>
@endsection
